...
//  optionTMonad[List].pure(42) should be(OptionT(List(Some(42))))
  ...
  test("Either") {
    ...
    right1.flatMap(x => Either.right(x + 1)) should be(Right(6))

    val left1: Either[String, Int] = Either.left("Something went wrong")
    left1.flatMap(x => Either.right(x + 1)) should be(Left("Something went wrong"))
    ...
    val right3: Either[String, Int] = Right(41)
    right3.map(_ + 1) should be(Right(42))

    val left3: Either[String, Int] = Left("Hello")
    left3.map(_ + 1) should be(Left("Hello"))
    left3.leftMap(_.reverse) should be(Left("olleH"))

    Either.catchOnly[NumberFormatException]("abc".toInt).isRight should be(false)
    Either.catchNonFatal(1 / 0).isLeft should be(true)

    val right4: Either[String, Int] = 42.asRight[String]
    right4 should be(Right(42))
  }
}
