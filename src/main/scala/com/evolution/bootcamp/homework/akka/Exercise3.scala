package com.evolution.bootcamp.homework.akka
...
object Exercise3 extends App {
  ...
  final class EntityRouterActor(
    ...
    private def working: Receive = {
      case command @ extractEntityId(_) =>
        val entityId = extractEntityId(command)
        entityRefs.getOrElse(entityId, startEntity(entityId)) ! command
    ...
    private def startEntity(id: String): ActorRef = {
      val ref = context.actorOf(entityProps, ActorNameSanitizer.sanitize(id))
      context.watch(ref)
      entityRefs += (id -> ref)
      ref
    }

    private def stopEntities(): Unit = entityRefs.values.foreach(_ ! PoisonPill)

    private def stopRouter(): Unit = if (entityRefs.isEmpty) context.stop(self)

    private def deleteEntity(entity: ActorRef): Unit =
      entityRefs = entityRefs.filterNot { case (_, ref) => ref eq entity }
